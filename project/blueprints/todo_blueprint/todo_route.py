...
def get_all_member_todos(current_member):
    try:
        ...
        return make_response(
            { "Todos" : [todo.serialize for todo in Todos] }, 200
        )
    except Exception as e:
        return make_response(
            { "message" : "Sorry no todo found." }, 400
        )
...
def create_member_todo(current_member):
    ...
    if not description and urgency:
        return make_response(
            {
                "message" : "Invalid todo. Please complete all fields."
            },400
        )
    ...
    try:
        ...
        return make_response(
            { "message": "Success! Todo has been created"}, 200
        )
    except Exception as e:
        return make_response(
            { "message" : "Opps! Todo can't be created." }, 400
        )
...
def get_all_family_todos(current_member):
    try:
        ...
        return make_response(
            {"Family Todos": [todo.serialize for todo in Todos]}, 200
        )
    except Exception as e:
        return make_response(
            { "message" : "Sorry no todo found." }, 400
...
def update_todo_by_id(current_member, id):
    ...
    try:
        TodoItem.query.filter_by(id = id).update(data)

        db.session.commit()

        return make_response(
            { "message": "Success! Todo has been updated"}, 200
        )
    except Exception as e:
        return make_response(
            { "message" : "Opps! Todo can't be updated." }, 400
        )
